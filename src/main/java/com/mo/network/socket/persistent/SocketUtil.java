...
import com.mo.network.util.ByteUtil;
...
public class SocketUtil {
     ...
     * @param socket
     * @param data   要传送的数据字节
     * @throws IOException 发送失败时（比如socket意外关闭时）抛出异常
     */
     ...
     *
     * @param socket
     * @return 返回去除包装的有效数据字节，如果是心跳（数据长度为0）则返回null
     * @throws IOException 读取失败时（比如socket意外关闭时）抛出异常
     */
